...
export default function Header() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile menu */}
      <div
        className={`fixed inset-0 top-[60px] z-40 bg-sam-green/98 backdrop-blur-sm transition-all duration-300 lg:hidden ${
          mobileOpen ? 'visible opacity-100' : 'invisible opacity-0 pointer-events-none'
        }`}
      >
        <nav className="flex flex-col items-center gap-6 px-6 py-10" aria-label="Menu mobile">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
